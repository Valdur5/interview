...
public class FindPathsWithSumInBinGraphImpl {
     ...
     * @param treeGraph the root of the graph.
     * @param sum the sum we are searching paths for.
     * @return the list of possible paths in the graph to reach the sum.
     */
    public List<List<Integer>> findPathsWithSumInGraph(TreeGraph treeGraph, int sum) {
        List<List<Integer>> paths = new ArrayList<>();
        
        findPath(treeGraph, paths, new ArrayList<>(), sum);
     ...
     * @param treeGraph the node of the graph which should be checked, first this is the real root, late this point moves through the graph.
     ...
     */
    private void findPath(TreeGraph treeGraph, List<List<Integer>> paths, ArrayList<Integer> currentPath, int sum) {
        if(treeGraph == null) {
            return;
        }

        // Add the current node to the current Path.
        currentPath.add(treeGraph.getValue());
        ...
        if(treeGraph.getLeft() == null && treeGraph.getRight() == null && treeGraph.getValue() == sum) {
        ...
        findPath(treeGraph.getLeft(), paths, new ArrayList<>(currentPath), sum-treeGraph.getValue());
        findPath(treeGraph.getRight(), paths, new ArrayList<>(currentPath), sum-treeGraph.getValue());
    }
}
